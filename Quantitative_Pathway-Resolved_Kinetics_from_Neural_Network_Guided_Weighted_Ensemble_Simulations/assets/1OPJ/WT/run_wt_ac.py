...
from omm_1opj_wt import OpenMMRunner
...
class PathCV:
    ...
    def _check_equidistance(self, *, enforce, tol):
        ...
        if rel > tol:
            msg = (
                f"Path nodes are not equidistant (std/mean = {rel:.2f}). This violates assumptions of Branduardi et al."
            )
            if enforce:
...
def extract_coords_from_array(
    ...
    *,
    ...
    R, rmsd = rotation_matrix(mob_bb_c, ref_bb_c, weights=bb_masses)
...
if __name__ == "__main__":
    ...
    initial_positions = []
    for i in range(135):
        u = mda.Universe(f"WT/1OPJ_WT_ac_path/frame_{i}.gro")
    ...
    points = np.load("WT/1OPJ_WT_ac_path/ac_path.npy")

    path_cv = PathCV(points, mass_weights=None, enforce_equidistance=True, equidistance_tol=1.50, normalize_output=True)
    ...
    config = {
        ...
        "n_walkers_per_bin": 3,
        "dt": 0.004,
        "n_steps_per_tau": 10000,
        "n_iterations": 50000,
        "flux_file": "wt_flux_ac_algebric_clean1.txt",
        "survive_empty": False,
    ...
    we_sim = WESS(
        config=config, initial_positions=initial_positions, projection_fn=projection_fn, kwargs=projection_kwargs
    )
    we_sim.run()
