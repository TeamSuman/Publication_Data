...
parser.add_argument('--index', required=False, default=1, help="Index")
...
def collate(samples):
    ...
    solute_len_matrix = get_len_matrix(solute_graphs.batch_num_nodes().to("cpu").numpy())
    solvent_len_matrix = get_len_matrix(solvent_graphs.batch_num_nodes().to("cpu").numpy())
...
def main():
    train_df = pd.read_csv(f'/home/suman/Dibyendu/Projects/SERB/DeltaG_Prediction/DataSets/Full_Free_Solv/train{index}.csv', sep=";")
    valid_df = pd.read_csv(f'/home/suman/Dibyendu/Projects/SERB/DeltaG_Prediction/DataSets/Full_Free_Solv/test{index}.csv', sep=";")
